defmodule DatagroutConduit.Transport.MCP do
  @moduledoc """
  MCP Streamable HTTP transport.

  Sends HTTP POST requests to the MCP endpoint with JSON-RPC 2.0 bodies
  and MCP-specific headers. Parses SSE or direct JSON responses.
  """
  ...
  defp do_send_request(req, opts, _retried: retried) do
    ...
    case Req.post(req, url: "", json: body, headers: headers) do
      {:ok, %Req.Response{status: 202, headers: resp_headers}} ->
        new_session_id = get_header(resp_headers, "mcp-session-id")
        {:ok, %{"accepted" => true}, new_session_id}

      {:ok, %Req.Response{status: status, body: response_body, headers: resp_headers}}
      when status in 200..299 ->
        new_session_id = get_header(resp_headers, "mcp-session-id")

        case parse_response(response_body, resp_headers, opts.id) do
          {:ok, result} -> {:ok, result, new_session_id}
          {:error, _} = err -> err
        end
  ...
  defp parse_response(body, headers, request_id) when is_binary(body) do
    content_type =
      headers
      |> Enum.find_value("", fn
        {"content-type", v} -> v
  ...
  defp parse_response(body, _headers, _request_id) when is_map(body) do
    extract_result(body)
  end

  defp parse_response(body, _headers, _request_id) when is_list(body) do
    case List.last(body) do
      nil -> {:error, :empty_response}
      item -> extract_result(item)
    end
  end

  defp parse_sse(data, _request_id) do
    events =
      data
      |> String.split("\n\n", trim: true)
    ...
    case List.last(events) do
      nil -> {:error, :no_sse_data}
      event -> extract_result(event)
    end
  end
